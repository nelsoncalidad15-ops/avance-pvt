...
export const ChartWrapper = ({ title, subtitle, children, className, action, isDark = false }: { title: string, subtitle?: string, children: React.ReactNode, className?: string, action?: React.ReactNode, isDark?: boolean }) => {
  ...
  return (
    <div className={`${isDark ? 'bg-slate-900 border-white/10 shadow-2xl' : 'bg-white border-slate-100/60 shadow-[0_8px_30px_rgb(0,0,0,0.02)]'} rounded-[2rem] border p-8 relative group transition-all hover:shadow-[0_12px_40px_rgb(0,0,0,0.04)] flex flex-col ${className || ''}`}>
      ...
      <div ref={chartRef} className={`flex-1 min-h-0 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
        {children}
      </div>
    </div>
  );
...
export const LuxuryKPICard = ({ title, value, color, icon: Icon, trend, isDark = false, isDanger = false, breakdown }: { 
  title: string, 
  value: string | number, 
  color: string, 
  icon: any,
  ...
  <motion.div 
    ...
    animate={isDanger ? { 
      boxShadow: ["0 0 0px rgba(225,29,72,0)", "0 0 15px rgba(225,29,72,0.2)", "0 0 0px rgba(225,29,72,0)"],
      borderColor: ["rgba(225,29,72,0.1)", "rgba(225,29,72,0.4)", "rgba(225,29,72,0.1)"]
    } : {}}
    transition={isDanger ? { duration: 2, repeat: Infinity } : {}}
    className={`${isDark ? 'bg-slate-900 border-white/10 shadow-2xl' : 'bg-white border-slate-100 shadow-sm'} 
      ${isDanger ? 'border-rose-500/40 bg-rose-50/5' : ''}
      p-6 rounded-[2rem] border hover:shadow-md transition-all group overflow-hidden relative flex flex-col h-full`}
  >
    <div className={`absolute -right-12 -top-12 w-40 h-40 rounded-full opacity-[0.02] group-hover:opacity-[0.05] transition-all duration-1000 ${color}`}></div>
    
    <div className="relative z-10 flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 rounded-xl ${isDanger ? 'bg-rose-600 text-white animate-pulse' : `${color.replace('bg-', 'bg-').replace('-600', '-50')} ${color.replace('bg-', 'text-')}`} shadow-sm`}>
            <Icon className="w-4 h-4" />
          </div>
          <p className={`text-[9px] font-black ${isDark ? 'text-slate-500' : 'text-slate-400'} uppercase tracking-[0.2em]`}>{title}</p>
        </div>
        {trend && (
          <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${trend.isUp ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'} border border-current/10`}>
            {trend.isUp ? <Icons.ArrowUp className="w-2.5 h-2.5" /> : <Icons.ArrowDown className="w-2.5 h-2.5" />}
            {trend.value}%
          </div>
        )}
      </div>
            ...
            {breakdown.map((item, i) => (
              <div key={i} className="group/item">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate mr-2">{item.name}</span>
                  <div className="text-right flex flex-col items-end">
                    <span className={`text-xl font-black italic leading-none ${isDark ? 'text-white' : 'text-slate-900'} ${isDanger && item.percentage === 100 ? 'text-rose-600' : ''}`}>{item.value}</span>
                    {item.secondaryValue && (
      ...
      <div className="pt-4 border-t border-slate-50 mt-auto">
        <div className="flex justify-between items-end">
          <div>
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Consolidado</span>
            <h4 className={`text-lg font-black ${isDark ? 'text-white' : 'text-slate-900'} tracking-tighter leading-none italic opacity-40`}>
